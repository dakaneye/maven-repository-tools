...
public class Configuration {
    ...
    @Parameter(
            names = {"-s", "-sourceUrl"},
            description = "URL for the source repository from which artifacts are resolved, "
    ...
    @Parameter(
            names = {"-su", "-sourceUsername"},
            description = "Username for the source repository, if required.")
    private String sourceUsername;

    @Parameter(
            names = {"-sp", "-sourcePassword"},
            description = "Password for the source repository, if required.")
    private String sourcePassword;

    @Parameter(
            names = {"-is", "-includeSources"},
            description = "Flag to enable/disable download of sources artifacts.",
            arity = 1)
    private Boolean includeSources = true;

    @Parameter(
            names = {"-ij", "-includeJavadoc"},
            description = "Flag to enable/disable download of javadoc artifacts.",
            arity = 1)
    private Boolean includeJavadoc = true;

    @Parameter(
            names = {"-ip", "-includeProvidedScope"},
            description = "Flag to include/exclude provisioning of dependencies with provided scope.",
            arity = 1)
    private Boolean includeProvidedScope = false;

    @Parameter(
            names = {"-it", "-includeTestScope"},
            description = "Flag to include/exclude provisioning of dependencies with test scope.",
            arity = 1)
    private Boolean includeTestScope = false;

    @Parameter(
            names = {"-ir", "-includeRuntimeScope"},
            description = "Flag to include/exclude provisioning of dependencies with runtime scope.",
            arity = 1)
